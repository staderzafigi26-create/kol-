...
async function loadCreatorLocations() {
  try {
    const response = await fetch('/data/creator-locations.json', { cache: 'no-store' });
...
async function ensureWorldGeo() {
  ...
  worldGeoReady = fetch('/data/world.json', { cache: 'force-cache' })
    .then((response) => {
...
async function ensureUsaGeo() {
  ...
  usaGeoReady = fetch('/data/usa.json', { cache: 'force-cache' })
    .then((response) => {
...
async function loadCenter() {
  centerEls.status.textContent = '正在加载本地中台数据...';
  ...
  renderTables();
  centerEls.status.textContent = '本地中台已加载。';
}
